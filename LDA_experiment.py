...
class SmoothedLdaModel:
    def __init__(self, docs, num_topics, vocabulary):
        ...
        for d, n in enumerate(self.N):
            self.phi[d, n:, :] = 0
        self.gamma = self.alpha + np.ones((self.M, self.K)) * self.N.reshape(-1, 1) / self.K
        self._lambda = np.random.gamma(shape=100, scale=0.01, size=(self.K, self.V))
        self.eta = np.ones(self.V)

    def e_step(self, iterations=50):
        for d in range(self.M):
            # update phi
            self.phi[d, :self.N[d], :] = np.exp(
                psi(self.gamma[d, :]) - psi(self.gamma[d, :].sum()) \
                + psi(self._lambda[:, self.docs[d]]).T - psi(self._lambda[:, self.docs[d]].sum(axis=1))
            )

            # update lambda
            for i in range(self.V):
                self._lambda[:, i] = np.array([
                    (np.array(self.docs[d]) == i) @ self.phi[d, :self.N[d], :] for d in range(self.M)
                ]).sum(axis=0)
            self._lambda += self.eta
    ...
    def m_step(self):
        # update alpha and eta: Newton-Raphson
        self.newton_raphson()
    ...
    def newton_raphson(self, max_iter=10000, eps=1e-2):
        ...
        for _ in range(max_iter):
            eta = self.eta.copy()
            # gradient
            g = self.K * (psi(self.eta.sum()) - psi(self.eta)) \
                + (psi(self._lambda) - psi(self._lambda.sum(axis=1)).reshape(-1, 1)).sum(axis=0)
            # hessian matrix: H = diag(h) + 1z1^T
            # Compute Newton step: d = -(H^-1g) = -(g-c)/h
            z = self.K * polygamma(1, self.alpha.sum())
            h = -self.K * polygamma(1, self.alpha)
            c = (g / h).sum() / (1./z + (1./h).sum())
            d = -(g - c) / h
            self.eta += d

            # check convergence
            err = np.linalg.norm(self.eta - eta)
            # print(f"{_}", err)
            if  err < eps:
                break
    ...
    def elbo(self):
        lb = 0
        for d in range(self.M):
            lb += self.K*(loggamma(np.sum(self.eta)) - np.sum(loggamma(self.eta))) \
                + ((self.eta - 1)*(psi(self._lambda) - psi(self._lambda.sum(axis=1)))).sum()

            lb += loggamma(self.alpha.sum()) - loggamma(self.alpha).sum() \
               + ((self.alpha - 1)*(psi(self.gamma[d, :]) - psi(self.gamma[d, :].sum()))).sum()

            lb += (self.phi[d, :self.N[d], :] * (psi(self.gamma[d, :]) - psi(self.gamma[d, :].sum()))).sum(axis=1).sum(axis=0)

            lb += (self.phi[d, :self.N[d], :] * np.log(self._lambda[:, self.docs[d]].T)).sum(axis=1).sum(axis=0)
